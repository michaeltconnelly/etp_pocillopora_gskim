...
# MODIFIED: Michael Connelly, Oct. 2024
# USAGE: perl angsd_step3_processoutput.pl Dxy_persite.txt WindowSize(Int)
#
# ##############################################################
use strict;

my $totalLen = 361425095; #For all P. grandis chromosomes/unplaced contigs. Adjust if running on other chromosomes
my $win = "$ARGV[1]"; 

my $chr = "$ARGV[0]";
my $output = "chr_angsd_DxySummary\.txt";
...
for (my $i = 1; $i <= ($totalLen - $win); $i += $win){
	...
	print OUT "chr_X\t$start\t$end\t$avg\t$avgSitesWithData\n";
}
close OUT;
