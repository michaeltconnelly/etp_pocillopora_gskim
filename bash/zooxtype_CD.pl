...
symgenomes=11-12 : range of \'chr\' numbers containing concatenated symbiont references
                   in the order C, D
...
sample\tsitesHost\tsitesC\tsitesD\tcountHost\tcountC\tcountD\tfracZoox\tfracC\tfracD
...
print "sample\tsitesH\tsitesC\tsitesD\tcountH\tcountC\tcountD\tfracZ\tfracC\tfracD\n";
...
#warn "$file:A:Q=$qual:S=$site\n$_";
	...
	my $sumf=$nC+$nD;
	if ($sumf==0) {
		print "$file\t$sH\t$sC\t$sD\t$nH\t$nC\t$nD\tNA\tNA\tNA\n";
		next;
	}
	my $fZ=sprintf("%.5f",$sumf/($sumf+$nH));
	$fC=sprintf("%.3f",$nC/$sumf);
	$fD=sprintf("%.3f",$nD/$sumf);
 	print "$file\t$sH\t$sC\t$sD\t$nH\t$nC\t$nD\t$fZ\t$fC\t$fD\n";
 }
