#!/bin/bash
#./bash/samtools_symbiontCD_fasta_jellyfish_wrapper.sh
...
do \
JOBFILE="${prodir}/bash/jobs/${sample}_symCD_samtools_split.job"
touch $JOBFILE
#
...
echo "#$ -j y
#$ -N ${sample}_symCD_samtools_jellyfish
#$ -o ${prodir}/bash/jobs/${sample}_symCD_samtools_jellyfish.log
#$ -m bea
...
#   input command to produce fasta files for both Cladocopium and Durusdinium alignments
echo "samtools fasta ${prodir}/outputs/symbiont_alignments/${sample}_symCD.sorted.bam \
-o ${prodir}/outputs/symbiont_kmers/${sample}_symCD.fasta -@ 8"  >> $JOBFILE

#   input command for jellyfish kmer counting
echo "jellyfish count -m 21 -s 10000000 ${prodir}/outputs/symbiont_kmers/${sample}_symCD.fasta \
