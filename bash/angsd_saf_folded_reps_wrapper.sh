#!/bin/bash
#./bash/angsd_saf_folded_reps_wrapper.sh
...
ls ${prodir}/outputs/alignments/pgra_himb/*md.rg.bam | grep -f ${prodir}/data/pops_ngsadmix/${set} > ${angsddir}/bamfiles/${set}_reps_bamfile.txt
...
JOBFILE="${prodir}/bash/jobs/angsd_saf_1d-folded_reps_${set}.job"
touch $JOBFILE
# input QSUB commands
echo "#!/bin/sh
# ----------------Parameters---------------------- #
...
angsd -sites ${angsddir}/AllSites_Pgra_HIMB.txt -rf ${angsddir}/chrs_Pgra_HIMB.txt -b $BAMS -anc $ANC -GL 1 -P $NSLOTS $TODO -out ${angsddir}/safs/${set}.reps.folded 
echo "DONE!"' >> $JOBFILE
