...
#   input QSUB commands
echo "# /bin/sh" > ${prodir}/bash/jobs/${sample}_bwa_align.job
echo "# ----------------Parameters---------------------- #" >> ${prodir}/bash/jobs/${sample}_bwa_align.job
echo "#$  -S /bin/sh
#$ -pe mthread 8
#$ -q mThM.q
#$ -l mres=96G,h_data=12G,h_vmem=12G,himem" >> ${prodir}/bash/jobs/${sample}_bwa_align.job
echo "#$ -j y
#$ -N ${sample}_bwa_align
#$ -o ${prodir}/bash/jobs/${sample}_bwa_align.log
#$ -m bea
#$ -M connellym@si.edu" >> ${prodir}/bash/jobs/${sample}_bwa_align.job
#
echo "# ----------------Modules------------------------- #" >> ${prodir}/bash/jobs/${sample}_bwa_align.job
echo "module load bioinformatics/bwa" >> ${prodir}/bash/jobs/${sample}_bwa_align.job
#
echo "# ----------------Your Commands------------------- #" >> ${prodir}/bash/jobs/${sample}_bwa_align.job
#
echo 'echo + `date` job $JOB_NAME started in $QUEUE with jobID=$JOB_ID on $HOSTNAME' >> ${prodir}/bash/jobs/${sample}_bwa_align.job
...
#
echo 'echo '${sample}' successfully aligned' >> "${prodir}"/bash/jobs/${sample}_bwa_align.job
#
echo 'echo = `date` job $JOB_NAME done' >> ${prodir}/bash/jobs/${sample}_bwa_align.job
# submit job
