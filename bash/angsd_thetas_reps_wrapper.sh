#!/bin/bash
#./bash/angsd_thetas_reps_wrapper.sh
...
JOBFILE="${prodir}/bash/jobs/angsd_thetas_reps_${pop1}.job"
touch $JOBFILE
# input QSUB commands
echo "#!/bin/sh
# ----------------Parameters---------------------- #
...
#$ -o ${prodir}/bash/jobs/angsd_thetas_reps_${pop1}.log
#$ -m bea
#$ -M connellym@si.edu
#
# ----------------Modules------------------------- #
...
printf '/share/apps/bioinformatics/angsd/0.941/angsd/misc/realSFS saf2theta ${angsddir}/safs/%s.reps.folded.saf.idx -sfs ${angsddir}/1dsfs/%s.reps.folded.ml -outname ${angsddir}/thetas/%s.reps -fold 1 \n' "$pop1" "$pop1" "$pop1" >> $JOBFILE
# estimate theta for every Chromosome/scaffold
printf '/share/apps/bioinformatics/angsd/0.941/angsd/misc/thetaStat do_stat ${angsddir}/thetas/%s.reps.thetas.idx -outnames ${angsddir}/thetas/%s.reps.thetas \n' "$pop1" "$pop1" >> $JOBFILE
