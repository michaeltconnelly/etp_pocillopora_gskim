#!/bin/bash
#./bash/angsd_dxy_mafs_reps_wrapper.sh
...
# ls ${prodir}/outputs/alignments/pgra_himb/*md.rg.bam | grep -f ${prodir}/data/pops_ngsadmix/${set} > ${angsddir}/bamfiles/${set}_all_bamfile.txt
# verify sample numbers are correct
echo "For ${set}, There are $(cat ${angsddir}/bamfiles/${set}_reps_bamfile.txt | wc -l) samples in total"
# create job file
echo "Creating job file for population-specific MAF estimation of ${set}"
JOBFILE="${prodir}/bash/jobs/angsd_dxy_mafs_reps_${set}.job"
touch $JOBFILE
# input QSUB commands
echo "#!/bin/sh
# ----------------Parameters---------------------- #
...
#$ -cwd
#$ -j y
#$ -N angsd_dxy_mafs_reps_${set}
#$ -o ${prodir}/bash/jobs/angsd_dxy_mafs_reps_${set}.log
#$ -m bea
#$ -M connellym@si.edu
#
# ----------------Modules------------------------- #
...
BAMS="${angsddir}/bamfiles/${set}_reps_bamfile.txt" 
...
angsd -sites ${angsddir}/AllSites_Pgra_HIMB.txt -rf ${angsddir}/chrs_Pgra_HIMB.txt -b $BAMS -ref $REF -GL 1 -P $NSLOTS $TODO -out ${angsddir}/mafs/${set}.reps
