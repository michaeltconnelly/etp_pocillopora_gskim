...
```{r pca}
# pop<-read.table("Demo/Data/Demo1pop.info")
C <- as.matrix(read.table("./outputs/angsd/all_noclones.cov"))
e <- eigen(C)
```
```{r pca_test_plot}
plot(e$vectors[,1:2],xlab="PC1",ylab="PC2", main="individual allele frequency")
...
```{r}
pdf(file="./outputs/figures/nobones/pcangsd_species.pdf", width = 5.5, height = 3.75)
title <- expression(paste("East Pacific ", italic("Pocillopora"), " PCA (4,644,899 SNPs)"))
pcs %>% ggplot(aes(x = V1, y = V2)) +
