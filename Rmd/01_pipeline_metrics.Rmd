...
```{r}
multiqc_flagstat_comparison_tidy <- multiqc_flagstat_comparison %>% pivot_longer(cols = c("pdam_rsmas", "pver_kaust", "pver_csiro"), names_to = "reference", values_to = "mapped_reads")
```

```{r}
multiqc_flagstat_comparison_tidy %>% ggplot(aes(`reference`, `mapped_reads`)) + geom_violin()
```
```{r}
...
samples_rep_5X <- left_join(qpop_rep_samples, qc_5X, by = "Sample ID") %>% 
  group_by(pop) %>% slice_max(`5X Coverage`, n = 10) %>% arrange(pop, desc(`5X Coverage`)) %>% ungroup() %>%  select(`Sample ID`)
