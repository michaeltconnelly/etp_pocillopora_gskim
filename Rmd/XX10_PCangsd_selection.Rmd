...
highly_distinct_genes <- outlier_site_genes %>% 
  filter(qval < 1e-10) %>% filter(grepl("OZ", chr)) %>% 
  group_by(ID) %>%
  mutate(n = n()) %>%
  # arrange(desc(n)) %>%
  distinct(ID, .keep_all = T) %>% 
...
top_distinct_genes_chr <- highly_distinct_genes %>% group_by(chr) %>% slice_min(order_by = qval, n = 100)
```
...
```{r}
topGOtest <- topGOanalyzeDE(highly_distinct_genes) 
```
