...
```
```{r}
pop_fst <- read_csv("./outputs/pgrandis_population_fst.csv")
```
...
fst_heatmap <- ggplot(data = pop_fst, aes(pop1, pop2, fill = Fst)) +
  ...
  scale_fill_gradient(low = "white", high = "red", limits = c(0,0.08), name="FST") +
  geom_text(aes(label = round(Fst, 3)), size = 2) +
  theme(axis.title = element_blank(),
        axis.text.x = element_text(angle = 90, vjust = -1, size = 10, hjust = 0), 
        axis.text.y = element_text(size = 10)) + 
  coord_fixed() + scale_x_discrete(position = "top")
print(fst_heatmap)
```
```{r}
pdf("./outputs/figures/pgrandis_fst_heatmap.pdf", height = 5, width = 5.75)
...
### P.grandis populations
```{r}
fst_files <- list.files("./outputs/angsd/10kb_fst/pgrandis_pops", full.names = T)
for (file in fst_files) { 
  fst_df <- read_delim(file, delim = "\t", skip = 1, col_names = angsd_fst_colnames, col_types = angsd_fst_cols)
  #
  fst_hist <- fst_df %>% ggplot(aes(Fst)) + 
