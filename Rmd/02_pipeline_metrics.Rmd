...
# 314,355,341
sum(multiqc_unmapqc[multiqc_unmapqc$`Sample ID` %in% samples_noclones$`Sample ID`,]$`Total Sequences`)
...
multiqc_data  %>% 
  ggplot(aes(`Site`, `Unmapped Proportion`)) + geom_violin()
# 
# Bahia Culebra has highest proportion of unmapped reads (~50%), other regions are between 3-8%
```
```{r}
range(multiqc_data$`Unmapped Reads`)
mean(multiqc_data$`Unmapped Reads`)
sd(multiqc_data$`Unmapped Reads`)

mean(multiqc_data$`Unmapped Proportion`)
sd(multiqc_data$`Unmapped Proportion`)
range(multiqc_data$`Unmapped Proportion`)
```
### Symbiont mapped reads

# samples for Raul collaboratoin

```{r}
samples_stri_panama_cd_categories %>% left_join(multiqc_fastqc_reads, multiqc_trimqc_reads, by = "Sample ID") %>%
  left_join(., multiqc_flagstat_reads, by = "Sample ID") %>% 
  left_join(., multiqc_unmapqc_reads, by = "Sample ID") %>% 
  filter(Region == "Panama" & `Symbiont Community` != "Mixed" & Site != "Saboga") %>% 
  # dplyr::relocate(`Sample ID`:`Unmapped Reads`) %>%
  dplyr::select(`Sample ID`, `Site`, `mtorf_type`, `Species`, `Symbiont Community`:`Unmapped Reads`, `sitesC`:`sitesD`, `countC`:`countD`, `fracC`:`fracD`) %>%
  group_by(`Site`,`Species`, `Symbiont Community`) %>% 
  top_n(1, `Unmapped Reads`) %>%
  write_excel_csv("~/Desktop/panama_samples_subset_symbiont_metrics.csv")
```
