...
## Perform separate PCAs for Cluster 1, 2, and 3 samples

### Plot PCAs for Cluster 1
```{r pca_c1}
# pop<-read.table("Demo/Data/Demo1pop.info")
C1 <- as.matrix(read.table("./outputs/angsd/etp_cluster1.cov"))
e1 <- eigen(C1)
```
```{r pca_test_plot}
...
```{r combine_pca_metadata}
# combine pc coordinates with sample metadata
pcs_c1 <- as.data.frame(e1$vectors) %>% cbind(samples_cluster1)
# store variable with % variance explained by each PC
