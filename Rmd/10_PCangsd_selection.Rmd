...
```{r}
gene_coordinates <- read_delim(file = "./data/pdam_genome_gene_coordinates.txt", col_names = c("chr","start","end","ID"))
...
ngsadmix_spp_lowercase <- c("peffusa", "pgrandiscontinent", "pgrandisoffshore", "pmeandrina", "pverrucosa3a", "pverrucosa3b")
#
...
```
```{r}
sqv <- pcadapt_site_qvals(spp_combos$comp[10], alpha = 0.05)
```
```{r}
...
```{r}
topGOtest <- topGOanalyzeDE(distinct_outlier_genes_list[[6]]) 
```
## Perform toGO tests 
