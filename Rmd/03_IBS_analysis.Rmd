...
---
Code for Panama *Pocillopora* genome skimming manuscript
...
bams <- read.table("/Users/mikeconnelly/computing/projects/etp_pocillopora_gskim/outputs/angsd/all_analysis_bamfile.txt",header=F)[,1]
#
...
```{r}
tree_title <- expression(paste("Identity-by-state clustering of all ", italic("Pocillopora"), " samples (n=", 289, ")"))
```
```{r ibs_ggtree}
...
hc %>% ggtree() %<+% samples + 
  ...
  theme_dendrogram(legend.position = c(0.8, 0.85),
        legend.margin = margin(0,0,0,0, unit = "mm"),
        plot.margin = margin(5,5,5,5, unit = "mm")) +
  ggtitle(expression(paste(italic("Pocillopora"), " identity-by-state clustering dendrogram")), subtitle = "All East Pacific samples (n= 289)")
# dev.off()
```
...
```{r}
pdf("./outputs/figures/nobones/ibs_dendrogram_all_analysis_black.pdf", width = 8.5, height = 4.85)
hc %>% ggtree() %<+% samples + 
  layout_dendrogram() + 
...
```{r}
pdf("./outputs/figures/nobones/ibs_dendrogram_all_analysis_mtorf.pdf", width = 8.5, height = 4.85)
hc %>% ggtree() %<+% samples + 
  layout_dendrogram() + 
  geom_tippoint(aes(fill = `mtorf_type`), shape = 21, color = "black", alpha = 0.8, stroke = 0.5, hjust = 0.3, show.legend = FALSE) +
  # geom_tiplab(size = 2, hjust = 1.05) +
...
```{r}
pdf("./outputs/figures/nobones/ibs_dendrogram_all_analysis_mtorf_replicates.pdf", width = 8.5, height = 4.85)
hc %>% ggtree() %<+% samples + 
  layout_dendrogram() + 
  geom_tippoint(aes(fill = `mtorf_type`, color = `REPLICATE`), shape = 21, alpha = 0.8, stroke = 0.5, hjust = 0.3, show.legend = FALSE) +
  scale_fill_manual(values = mtorf_colors, name = "mtORF haplotype") +
...
```{r}
ma <- as.matrix(read.table("./outputs/angsd/all_noclones_ibs05.ibsMat"))
```
```{r bamfile_sample_order_check}
# reading list of bam files = order of samples in IBS matrix
...
```
```{r}
pdf("./outputs/figures/nobones/ibs_dendrogram_all_noclones_mtorf.pdf", width = 8.5, height = 4.85)
hc %>% ggtree() %<+% samples + 
  layout_dendrogram() + 
  geom_tippoint(aes(fill = `mtorf_type`), shape = 21, color = "black", alpha = 0.8, stroke = 0.5, hjust = 0.3, show.legend = FALSE) +
  # geom_tiplab(size = 2, hjust = 1.05) +
