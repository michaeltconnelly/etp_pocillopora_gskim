...
# pdf(file="./outputs/figures/sicb/pcangsd_species.pdf", width = 5.5, height = 3.75)
pca <- pcs %>% ggplot(aes(x = V1, y = V2)) +
  geom_point(aes(fill = `NGSAdmix K6 Species`), shape = 21, size = 3, alpha = 0.75, show.legend = T) +
...
pca <- pcs %>% ggplot(aes(x = V2, y = V3)) +
  geom_point(aes(fill = `NGSAdmix K6 Species`), shape = 21, size = 3, alpha = 0.75, show.legend = F) +
  scale_fill_manual(values = ngsadmix_pop_colors_k6) +
  # facet_grid(Region ~ .) +
  # facet_wrap(mtorf_type ~ .) +
...
pca <- pcs %>%
  ...
  geom_point(aes(fill = `NGSAdmix K6 Species`), shape = 21, size = 4, alpha = 0.65, show.legend = T) +
  scale_fill_manual(values = ngsadmix_pop_colors_k6, name = "NGSAdmix Species (K=6)") + 
