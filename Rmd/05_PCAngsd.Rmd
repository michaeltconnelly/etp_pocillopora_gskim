...
```
```{r pca_3_4}
pca <- pcs %>% ggplot(aes(x = V3, y = V4)) +
  geom_point(aes(fill = `NGSAdmix K5 Species`), shape = 21, size = 3, alpha = 0.75, show.legend = F) +
  scale_fill_manual(values = ngsadmix_pop_colors_k6) +
  # facet_grid(Region ~ .) +
  # facet_wrap(mtorf_type ~ .) +
  coord_fixed(pcvar[4]/pcvar[3]) +
  xlab(paste0( "PC3 (", round(pcvar[3], 2), "%)")) + 
  ylab(paste0( "PC4 (", round(pcvar[4], 2), "%)")) +
  theme(plot.title = element_text(size = 16, hjust = -0.2),
        axis.title = element_text(size = 14),
        axis.text = element_text(size = 12),
        legend.title = element_text(size = 14),
...
pca12 <- pcs %>%
  ...
  geom_point(aes(fill = `NGSAdmix K5 Species`), shape = 21, size = 3, alpha = 0.65, show.legend = T) +
...
```{r}
pca34 <- pcs %>% ggplot(aes(x = V3, y = V4)) +
  geom_point(aes(fill = `NGSAdmix K5 Species`), shape = 21, size = 3, alpha = 0.75, show.legend = F) +
  scale_fill_manual(values = ngsadmix_pop_colors_k6) +
  # facet_grid(Region ~ .) +
  # facet_wrap(mtorf_type ~ .) +
...
```
```{r figS6_pca_linked}
pdf(file="./outputs/figures/FigS6_pcangsd_1234_linked_ngsadmix_species.pdf", width = 6.65, height = 5)
#
(free(pca12) / free(pca34)) & plot_annotation(tag_levels = "A") & 
...
pca <- pcs %>% ggplot(aes(x = V2, y = V3)) +
  ...
  xlab(paste0( "PC2 (", round(pcvar[2], 2), "%)")) + 
  ylab(paste0( "PC3 (", round(pcvar[3], 2), "%)")) +
  theme(plot.title = element_text(size = 16, hjust = -0.2),
        axis.title = element_text(size = 14),
        axis.text = element_text(size = 12),
        legend.title = element_text(size = 14),
...
### Figure 1A
```{r fig1A_pca}
pdf(file="./outputs/figures/Fig1A_pcangsd_ngsadmix_species_corrected.pdf", width = 6.65, height = 3.25)
#
...
## Figure S5
```{r fig5A_pca}
pdf(file="./outputs/figures/FigS5A_pcangsd_34_ngsadmix_species.pdf", width = 6.65, height = 2.15)
#
title <- expression(paste("East Pacific ", italic("Pocillopora"), " PCA (1,310,435 unlinked SNPs)"))
#
pca <- pcs %>%
...
#
plot(e1$vectors[,3:4],xlab="PC2",ylab="PC3", main="individual allele frequency")
# 
```
```{r bamfile_sample_order_check}
# reading list of bam files = order of samples in IBS matrix
...
## No-clones PCAngsd admixture (w/ linked SNPs)
```{r}
q <- read.table("./outputs/angsd/final_noclones_pgra_ibs05.admix.5.Q")
...
q_pop_tidy <- q_pop %>%
  ...
  pivot_longer(cols = `V1`:`V5`, names_to = "pop") %>%
...
```{r}
q <- read.table("./outputs/angsd/final_noclones_noLD_pgra_himb_filtered.admix.5.Q")
colnames(q) <- c("pop1", "pop2", "pop3", "pop4", "pop5")
...
q_pop_tidy <- q_pop %>%
  mutate("Sample ID" = fct_reorder(`Sample ID`, desc(Species))) %>% 
  pivot_longer(cols = `pop1`:`pop5`, names_to = "pop") %>%
  arrange(Region, mtorf_type)
#
#
q_pop_tidy_plot <- q_pop_tidy %>%
...
```
```{r barplot_k5}
# popcolors_k5 <- unname(ngsadmix_pop_colors_k6[c(2,3,4,5,1)])
popcolors_k5_pcangsd <- c("purple3", "gold2", "darkorange", "darkorange3", "turquoise2")
names(popcolors_k5) <- c("pop5", "pop1", "pop2", "pop3", "pop4")
...
# use Sample ID order saved for reordering factors 
# sample_order <- q_pop_k5$`Sample ID`

  ## ggplot attempt
sample_order_numeric <- match(q_pop_k5$`Sample ID`, sample_order)
# 
q_pop_tidy_k5 <- q_pop_k5 %>%
  ...
  mutate("pop" = factor(`pop`, levels = c("pop5", "pop1", "pop2", "pop3", "pop4")), ordered = T)


q_pop_tidy_plot_k5 <- q_pop_tidy_k5 %>%
  ggplot(aes(x = `Sample ID`, y = value)) +
  geom_col(aes(fill = `pop`), position = "stack", show.legend = F) +
  scale_fill_manual(values = popcolors_k5_pcangsd) +
...
## Figure S###
```{r}
pdf("./outputs/figures/FigS_pcangsd_K5_barplot.pdf", width = 6.65, height = 1.5)
q_pop_tidy_plot_k5 
dev.off()
```
