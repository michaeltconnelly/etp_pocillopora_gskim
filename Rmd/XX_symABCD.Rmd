...
pdam_cd_cats %>% group_by(`NGSAdmix K5 Species`, `Symbiont Community`) %>% count() %>% pivot_wider(names_from = `NGSAdmix K5 Species`, values_from = n)

symbiont_categories <- dplyr::select(pdam_cd_cats, `Sample ID`, `Symbiont Community`)
```
```{r}
# write list of C-dominated, D-dominated, and mixed samples for downstream analyses w/ ANGSD
c_dominated <- pdam_cd_cats %>% 
  filter(`Symbiont Community` == "C-dominated", NOCLONES_SAMPLE) %>%
  arrange(desc(fracC)) %>%
  dplyr::select(`Sample ID`)

d_dominated <- pdam_cd_cats %>% 
  filter(`Symbiont Community` == "D-dominated", NOCLONES_SAMPLE) %>%
  arrange(desc(fracD)) %>%
  dplyr::select(`Sample ID`)

write_lines(c_dominated$`Sample ID`, "C-dominated_samples.txt")

write_lines(d_dominated$`Sample ID`, "D-dominated_samples.txt")
```
