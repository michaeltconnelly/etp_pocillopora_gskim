...
#
costarica_samples <- read_csv("./data/pocillopora_samples_costarica_metadata.csv")
```
```{r}
...
regions_species <- samples %>% 
  ...
  # dplyr::rename("Type 1" = Type_1, "Type 3" = Type_3) %>% 
  mutate("sum" = (`P. effusa` + `P. grandis` + `P. meandrina` + `P. verrucosa 3a` + `P. verrucosa 3c`)) %>%
...
elevMapTest <-
  ...
  #
  geom_text_repel(aes(x = Longitude, y = Latitude, label = sum), size = 2.5, box.padding = 1, min.segment.length = 2, seed = 3, data = regions_species) +
  labs(x="Longitude", y="Latitude", elevation = "Elevation") +
  theme(panel.grid.major = element_blank(),
        panel.background = element_rect(fill = "#f0f8ff"),
        panel.border = element_rect(fill = NA),
  ...
  ggtitle(expression(italic("Pocillopora")*' sample origins and species identification'), subtitle = "All East Pacific samples (n= 289)")
elevMapTest
dev.off()
```
...
```{r figure 1A: powerpoint}
pdf(file="./outputs/figures/Pocillopora_origins_map_numbers.pdf", width = 8, height = 4)
par(mar = c(0.5, 0.5, 0.5, 0.5))
elevMapTest <-
  ggplot() +
  geom_sf(data = ocean, color = "black", size = 0.1, fill = "aliceblue") +
...
geom_point(aes(x = Longitude, y = Latitude, size = n),
  ...
  ggtitle(expression('East Pacific '*italic("Pocillopora")*' sample origins and numbers'))
elevMapTest
dev.off()
```
...
```
```{r}
pdf(file="./outputs/figures/Pocillopora_origins_map_numbersR1R2.pdf", width = 8, height = 4)
title <- expression('East Pacific '*italic("Pocillopora")*' sample origins')
par(mar = c(0.5, 0.5, 0.5, 0.5))
elevMapTest <-
  ggplot() +
  geom_sf(data = ocean, color = "black", size = 0.1, fill = "aliceblue") +
...
#
  geom_point(aes(x = Longitude, y = Latitude, size = n),
                    data = samples_regions_2, shape = 21, alpha = .8, fill = "red", color = "black", show.legend = FALSE) + 
  #
  scale_size_continuous(range = c(0.4, 10), limits = c(0, 60), breaks = c(5, 15, 30, 60), name = "# samples") +
  #
  labs(x="Longitude", y="Latitude") +
