...
# see example at: https://cran.r-project.org/web/packages/scatterpie/vignettes/scatterpie.html
#
regions_ngsadmix_pops <- samples_noclones_ngsadmix_meta %>%
  group_by(`Region`, `NGSAdmix Species`) %>% 
  dplyr::count() %>% 
  ...
  pivot_wider(names_from = `NGSAdmix Species`, values_from = n, values_fill = 0) %>%
...
elevMapTest <-
  ...
  scale_fill_manual(values = ngsadmix_pop_colors, name = "NGSadmix population") +
  #
...
#### Figure 1C
```{r}
pdf("./outputs/figures/Fig1C_ngsadmix_map.pdf", width = 8.5)
elevMapTest
dev.off()
```
