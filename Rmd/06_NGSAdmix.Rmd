...
samples_noclones_ngsadmix <- qpop_best %>% mutate(`NGSADMIX_REP_K5` = if_any(contains("pop"), representative),
                     `NGSAdmix K5 Species` = ifelse(representative(pop1), "P. effusa", 
...
popcolors_k5 <- unname(ngsadmix_pop_colors_k6[c(1,4,3,2,5)])
...
title <- expression(paste("NGSadmix results for K=5-6 on East Pacific ", italic("Pocillopora"), " (1,061,287 SNPs)"))
...
# create new metadata columns to identify representative vs. admixed samples and assign population labels
samples_k6_noadmix_assignments <- qpop_best %>% mutate(`NGSADMIX_REP_K6` = if_any(contains("pop"), representative),
                     `NGSAdmix K6 Species` = ifelse(representative(pop1), "P. grandis - Continent", 
                                           ifelse(representative(pop2), "P. meandrina",
                                                  ifelse(representative(pop3), "P. grandis - Offshore",
                                                         ifelse(representative(pop4), "P. effusa",
...
q_pop_k5 <- cbind(pop, q_k5) %>%
  ...
  left_join(samples_k6_noadmix_assignments, by = "Sample ID") %>% #samples_k6_assignments
...
q_pop_k5$`NGSAdmix K6 Spp Label` <- ngsadmix_spp[q_pop_k5$`NGSAdmix K6 Species`]
         
 q_pop_k5 <- q_pop_k5 %>% mutate(
         "Sample ID" = fct_reorder2(`Sample ID`, desc(`pop3`), desc(`pop4`)),
         "Sample ID" = fct_reorder2(`Sample ID`, `pop5`, desc(`pop2`)),
         "Sample ID" = fct_reorder(`Sample ID`, `NGSAdmix K6 Spp Label`),
         # 
         "Region" = factor(`Region`, levels = region_order_longitude, ordered = T)) %>% 
  arrange(`Sample ID`)
...
q_pop_tidy_k5 <- q_pop_k5 %>%
  ...
  mutate("pop" = factor(`pop`, levels = c("pop1", "pop4", "pop3", "pop2", "pop5"), ordered = T))
...
pdf("./outputs/figures/Fig1B_ngsadmix_K5-6_barplot.pdf", width = 6.65, height = 2)
q_pop_tidy_plot_k5 / q_pop_tidy_plot_k6
dev.off()
```
