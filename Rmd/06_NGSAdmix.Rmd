...
### Choose best K by Evanno
```{r}
bestKstats <- evanno_deltak("./outputs/ngsadmix/final_noclones/final_noclones_ngsadmix_likeKlogs.txt")
bestK <- bestKstats$K[which.max(bestKstats$DeltaK)]
```

```{r}
title <- expression("No-clones linked dataset: 229 samples, 5,587,480 SNPs")

bestKplot <- ggplot(bestKstats, aes(x = K, y = DeltaK)) +
  geom_vline(xintercept = bestK, color = "grey50") +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks = seq(4, 10, by = 1)) +
  labs(y = expression("Evanno's " * Delta * "K")) +
  ggtitle(title)
```
...
```{r}
bestKstats <- evanno_deltak("./outputs/ngsadmix/final_noclones/final_noclones_pgra_noLD_ngsadmix_likeKlogs.txt")
bestK <- bestKstats$K[which.max(bestKstats$DeltaK)]
```
...
bestKplot_noLD <- ggplot(bestKstats, aes(x = K, y = DeltaK)) +
  geom_vline(xintercept = bestK, color = "grey50") +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks = seq(4, 10, by = 1)) +
...
pdf("./outputs/figures/FigS7_ngsadmix_best_deltaK_plots.pdf", width = 6.5, height = 5.5)
print(figS7)
dev.off()
```
```{r bam_list}
...
plot_list[[3]] / plot_list[[4]] / plot_list[[5]] / plot_list [[6]] / plot_list[[7]] / plot_list[[8]] / plot_list[[9]] 
```
...
samples_noclones_ngsadmix_assignments_k5 <- qpop_best %>% mutate(`NGSADMIX_REP_K5` = if_any(contains("pop"), representative),
                     `NGSAdmix K5 Species` = ifelse(representative(pop1), "P. meandrina", 
                                           ifelse(representative(pop2), "P. effusa",
                                                  ifelse(representative(pop3), "P. capitata - Offshore",
                                                         ifelse(representative(pop4), "P. lacera",
                                                                ifelse(representative(pop5), "P. capitata - Continent", "Unassigned")))))) %>%
...
# pop1 = P. meandrina (10)
# pop2 = P. effusa (4)
# pop3 = Offshore P. capitata  (33)
# pop4 = P. lacera (33) 
# pop5 = Continental P. capitata (130) 
```
...
samples_k5_noadmix_assignments <- qpop_best %>% mutate(`NGSADMIX_REP_K5` = if_any(contains("pop"), representative),
                     `NGSAdmix K5 Species` = ifelse(representative(pop1), "P. meandrina", 
                                           ifelse(representative(pop2), "P. effusa",
                                                  ifelse(representative(pop3), "P. capitata - Offshore",
                                                         ifelse(representative(pop4), "P. lacera",
                                                                ifelse(representative(pop5), "P. capitata - Continent",
                                                                       "Unassigned")))))) %>%
  dplyr::select(`Sample ID`, `NGSADMIX_REP_K5`, `NGSAdmix K5 Species`)

ngsadmix_spp_code <- c("P. effusa" = "A",
                  "P. meandrina" = "B",
                  "P. capitata - Offshore" = "C",
                  "P. capitata - Continent" = "D",
                  "P. lacera" = "E")
```
...
# IMPORTANT: save Sample ID order for reordering factors in K = 6 plot and Fig S8
...
pdf("./outputs/figures/Fig1B_ngsadmix_K5_barplot_Nov2025_A.pdf", width = 6.65, height = 1.5)
q_pop_tidy_plot_k5 
dev.off()
```
```{r fig1B_ngsadmix_barplot_k5-6}
...
ngsadmix_pop_colors_k6 <- c("purple3", "gold2", "darkorange", "darkorange3", "turquoise2", "orangered2", "brown", "sandybrown")
...
popcolors_k8 <- unname(ngsadmix_pop_colors_k6)[c(4,2,8,5,6,3,7,1)]
names(popcolors_k8) <- c("pop1", "pop2", "pop3", "pop4", "pop5", "pop6", "pop7", "pop8")

popcolors_k8[4] <- "black"
  # 1,3
```
...
plot_k8 <- plot_list[[8]] + scale_fill_manual(values = popcolors_k8)
```

```{r figS8}
pdf("./outputs/figures/FigS8_ngsadmix_final_noclones_noLD_K3-8_Nov2025.pdf", width = 6.5, height = 8)

plot_k3 / plot_k4 / plot_k5 / plot_k6 / plot_k7 /plot_k8
...
## Extend cluster assignments back to full-clones dataframe (for summarizing clone groups across clusters)
```{r}
left_join(samples, samples_k5_assignments) %>% filter(CLONAL_GENOTYPE) %>% filter(!TECHNICAL_REPLICATE) %>%
  # filter(!is.na(`NGSAdmix K5 Species`)) %>% 
  View()
  
  group_by(`Genotype ID`, Country, Region, Site, Species, `NGSAdmix K5 Species`) %>%
  count() %>% ungroup() %>% group_by(Country, Region, Site, `NGSAdmix K5 Species`) %>% count() %>%   pivot_wider(names_from = `NGSAdmix K5 Species`, values_from = n) %>% 
  write_csv("outputs/clonegroups_per_cluster_site_spp.csv")

mutate(
    clonal_cluster = case_when(
      category == "A" ~ Genotype_ID == ,
      category == "B" ~ value1 + value2,
      TRUE ~ NA_real_ # For categories not matching "A" or "B"
    )
  )
```
