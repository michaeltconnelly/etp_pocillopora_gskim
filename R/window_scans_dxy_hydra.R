...
### -------------
dxy_all_out <- c(rep(-999, nrow(fst_df)))
dxy_est_out <- c(rep(-999, nrow(fst_df)))
wsize <- 50000
...
# calculate average Dxy across windows for all sites
  windf_all <- dxy_df[which(dxy_df$CHR == scaf & dxy_df$position >= start_pos & dxy_df$position <= end_pos), ] # & dxy$Dxy > 0 or 2e-6
  windf_all_len <- nrow(windf_all)
  dxy_all_out[j] <- (sum(windf_all$Dxy))/windf_all_len
  
# calculate average Dxy across windows for sites with Dxy estimate
  windf_est <- dxy_df[which(dxy_df$CHR == scaf & dxy_df$position >= start_pos & dxy_df$position <= end_pos) & dxy_df$Dxy > 0, ] 
  windf_est_len <- nrow(windf_est)
  dxy_est_out[j] <- (sum(windf_est$Dxy))/windf_est_len
  
}

#now add the dxy_all_out to the fst file:
fst_dxy_df <- fst_df
fst_dxy_df$DxyAllSites <- dxy_all_out
fst_dxy_df$DxyEstSites <- dxy_est_out
...
dxy_plot <- ggplot(fst_dxy_df, aes(wincenter, DxyAllSites)) + geom_point() + facet_grid(CHR ~ .)
